...
def read_file():
    data = pd.read_csv('chinanews33.csv',names=['category','theme','URL','content'])
...
def vec_transform(words):
    vec = CountVectorizer(analyzer="word",max_features=2500,lowercase=False)
...
def main():
    ...
    # vec_trian = vec_transform(words_train)#转化为向量格式
    # nbm = MultinomialNB()
    ...
    print("各类别数据总数:")
    print(df)
    print("分类准确率:"+str(score)) #0.8016
    print("训练时间:"+ str(round((end_1-start_1), 2)) + '秒')
    print("测试时间:"+ str(round((end_2-start_2), 2)) + '秒')

    #y_predict=nbm.predict(vec_trian.transform(words_test))
